            ...
            >
                <div className="mx-auto grid w-[min(92rem,calc(100%_-_3.2rem))] gap-4">
                    {news.length ? (
                        news.map((item) => (
                            <UI.Link
                                className="grid min-h-[15rem] gap-3 rounded-lg border border-slate-200 bg-white p-7"
                                href={`/news/${item.slug}`}
                                key={item.slug}
                            >
                                ...
                                <p className="m-0 leading-[1.7] text-slate-600">{item.summary ?? "상세 내용을 확인하세요."}</p>
                            </UI.Link>
                        ))
                    ) : (
