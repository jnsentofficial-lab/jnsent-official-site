...
export function SubPageSplit({ left, right, className = "", leftTabLabel = "좌측", rightTabLabel = "우측", rightPanelId }: SubPageSplitProps) {
        ...
        <SubPageSplitNavigationContext.Provider value={{ showRightPanel }}>
            ...
            >
                ...
                > */}
                    <button
                        type="button"
                        className={`mobile:fixed pc:hidden flex-1 rounded-full flex items-center gap-[0.4rem] ${IS_LEFT_SIDE_VIEW ? "flex-row" : "flex-row-reverse"} px-[2.0rem] py-[1.2rem] text-[1.6rem] font-[700] transition-colors fixed bottom-[2.4rem] right-[2.4rem] ${IS_LEFT_SIDE_VIEW ? "bg-[var(--adaptive-blue500)]" : "bg-[var(--adaptive-grey500)]"} z-[1000] text-white shadow-[0_0_100px_0_var(--adaptive-grey900)]`}
                        onClick={() => {
