...
const NumberColumn = ({ active, digit, className, rollingCount, size, delay, onClick }: NumberColumnProps) => {
    ...
    useEffect(() => {
        if (columnRef.current) {
            ...
            setPosition(active ? -height * (Math.max(1, rollingCount) * 10 + digit) : 0); // 음수로 내려가야 증가됨
        }
    }, [active, digit, rollingCount]);
    ...
    return (
        ...
        >
            <motion.div
                key="number-column"
                animate={{ y: position }}
                transition={{ duration: active ? Math.max(0.9, rollingCount * 0.45) : 0, delay: active ? delay : 0, ease: [0.25, 0.8, 0.25, 1], staggerChildren: 0.5 }}
