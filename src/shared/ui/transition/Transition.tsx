...
function FadeInOut({ activeKey, children, className = "", duration = 1.4, feather = 12 }: FadeInOutProps) {
    ...
    const [current, setCurrent] = useState<Layer>(initialCurrent);
    const [previous, setPrevious] = useState<Layer | null>(restoredPrevious);
    const [transitionId, setTransitionId] = useState(restoredPrevious ? 1 : 0);

    useEffect(() => {
        if (!restoredPrevious) return undefined;

        lastUnmountedLayer = null;
    ...
    return (
        <div
            ...
            data-transition="fade-in-out"
            data-transition-active-key={String(current.key)}
            data-transition-running={previous ? "true" : "false"}
        >
            <FadeLayer
                duration={duration}
                feather={feather}
            ...
            {previous ? (
                <div
                    ...
                    data-transition-previous={String(previous.key)}
                >
                    <FadeLayer
                        duration={duration}
                        feather={feather}
